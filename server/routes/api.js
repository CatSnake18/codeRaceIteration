...
router.get('/', snippetController.getCategories, (req, res) => {
...
router.post(
  ...
  (req, res) => res.status(200).json(res.locals.scoreBoardResponse),
);

//  this is the route used to populate our DB with snippets
//  no longer has use after popuatling SQL DB
router.post('/backdoor', snippetController.createDatabase, (req, res) =>
  res.status(200).send(),
);
